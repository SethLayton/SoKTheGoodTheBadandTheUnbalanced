...
if __name__ == '__main__':
    ...
    parser.add_argument('--eval_protocol_path', type=str, default="")
    # Dataset
    ...
    if mod_name and args.use_parallel:
        state_dict = torch.load(mod_name,map_location=device)
        ...
        print('Model loaded : {}'.format(mod_name))
    ...
    file_eval = genSpoof_list( dir_meta = args.eval_protocol_path,is_train=False,is_eval=True)
    print('no. of eval trials',len(file_eval))
    eval_set=Dataset_ASVspoof2021_eval(list_IDs = file_eval,base_dir = args.database_path)

    if args.designator == "pretrained":
        tmp = (args.eval_output).split(".score")
        scores_file = f"{tmp[0]}_p.score{tmp[1]}"
    else:
        scores_file = args.eval_output
