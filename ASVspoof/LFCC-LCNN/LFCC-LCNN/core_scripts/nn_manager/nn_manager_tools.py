...
def f_save_epoch_name(args, epoch_idx, suffix=''):
    ...
    """
    tmp_name = str(suffix) + "-" + "{}_{:03d}".format(args.save_epoch_name, epoch_idx) 
    return nii_str_tk.f_realpath(args.save_model_dir+'/models/{}'.format(str(suffix)), tmp_name, \
                                 args.save_model_ext)
...
def f_save_trained_name(args, suffix=''):
    ...
    """    
    return nii_str_tk.f_realpath(args.save_model_dir,args.iteration_numbers,".pt")
