...
def main():
    ...
    # print(os.getcwd())
    trn_lst = nii_list_tool.read_list_from_text((prj_conf.trn_list).format(args.iteration_numbers))
    trn_set = nii_dset.NIIDataSetLoader(
        (prj_conf.trn_set_name).format(args.iteration_numbers), \
        trn_lst,
        prj_conf.input_dirs, \
        prj_conf.input_exts, \
        prj_conf.input_dims, \
    ...
    # start training
    nii_nn_wrapper.f_train_wrapper(args, model,loss_wrapper, device,optimizer_wrapper,trn_set, val_set, checkpoint)
